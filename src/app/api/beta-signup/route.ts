...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.RESEND_API_KEY && process.env.RESEND_FROM_EMAIL) {
      ...
      const ageRange = escapeHtml(data.age_range);
      const householdType = escapeHtml(data.household_type);
      const policyCount = escapeHtml(data.policy_count);
      const policyTypes = data.policy_types.map(escapeHtml).join(", ");
      const frustration = data.biggest_frustration ? escapeHtml(data.biggest_frustration) : null;
      ...
      resend.emails.send({
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto; color: #1E293B;">
            <h1 style="color: #0D2A47; font-size: 24px;">Hi ${firstName}, you're on the list!</h1>
      ...
      resend.emails.send({
        ...
        subject: `New beta application — ${firstName} ${lastName}`,
        html: `
          <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto; color: #1E293B;">
            <h2 style="color: #0D2A47;">New beta application</h2>
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
              <tr><td style="padding: 6px 0; color: #64748B; width: 140px;">Name</td><td style="padding: 6px 0;">${firstName} ${lastName}</td></tr>
              <tr><td style="padding: 6px 0; color: #64748B;">Email</td><td style="padding: 6px 0;"><a href="mailto:${email}">${email}</a></td></tr>
              <tr><td style="padding: 6px 0; color: #64748B;">Age range</td><td style="padding: 6px 0;">${ageRange}</td></tr>
              <tr><td style="padding: 6px 0; color: #64748B;">Household</td><td style="padding: 6px 0;">${householdType}</td></tr>
              <tr><td style="padding: 6px 0; color: #64748B;">Policy count</td><td style="padding: 6px 0;">${policyCount}</td></tr>
              <tr><td style="padding: 6px 0; color: #64748B;">Policy types</td><td style="padding: 6px 0;">${policyTypes}</td></tr>
              ${frustration ? `<tr><td style="padding: 6px 0; color: #64748B; vertical-align: top;">Frustration</td><td style="padding: 6px 0;">${frustration}</td></tr>` : ""}
            </table>
            <hr style="border: none; border-top: 1px solid #E2E8F0; margin: 24px 0;" />
